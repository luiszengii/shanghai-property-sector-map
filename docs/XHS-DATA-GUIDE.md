...
| 地图新增但未采集 | 青浦、松江、金山 30 块、徐汇 12 块、长宁 7 块、静安—普陀 10 块，以及虹口—杨浦 6 块，共 65 块已登记为 `map_sectors_without_xhs_sample`；无样本、无分析结论、无代表来源 |
...
同日继续新增虹口四川北路、曲阳、凉城、江湾镇，以及杨浦控江路、五角场、新江湾城共 7 个低置信行政骨架候选。新江湾城与批次 D 已采集的同一市场身份一致，继续使用既有真实样本和分析；其余 6 个名称没有对应采集，只进入 `map_sectors_without_xhs_sample`。瑞虹新城、鲁迅公园、东外滩、鞍山、定海路、黄兴公园、中原尚未完成边界定义，不登记为已有地图板块，也不生成 XHS 空分析。
