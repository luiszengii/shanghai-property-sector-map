...
2026-07-30 用户明确允许现有安居客、房天下等快照在其受鉴权私有 Git 仓库
中作个人学习和跨设备同步。该决定只改变私有存储位置，不等同于平台书面数据
授权；这些快照仍不能公开再分发、进入生产、登记为可发布几何来源或替代
OSM/官方可复算输入。
...
现有 HFWGSJ 快照同样只允许在用户的私有 Git 仓库中作个人学习与比较，不得
进入公开产品或作为生成公开候选面的坐标来源。
